...
use crate::{catalog::ExtensionCatalog, error::HostError};
...
pub(crate) fn parse_toml<T>(text: &str, path: &Path) -> Result<T, HostError>
where
    T: serde::de::DeserializeOwned,
{
    toml::from_str(text).map_err(|source| HostError::toml(path.to_path_buf(), source))
}

impl BootstrapConfig {
    /// 从 TOML 文本解析；未知字段与缺字段直接失败。
    pub fn from_toml_str(text: &str) -> Result<Self, HostError> {
...
impl ExtensionsConfig {
    ...
    pub fn validate(&self, catalog: &ExtensionCatalog) -> Result<(), HostError> {
        ensure_version(self.version)?;
        let mut seen = HashSet::new();
        for entry in &self.extensions {
            if !seen.insert(entry.instance.as_str()) {
                return Err(HostError::DuplicateInstance {
                    instance: entry.instance.clone(),
                });
            }
            if catalog.get(&entry.factory).is_none() {
                return Err(HostError::UnknownFactory {
                    instance: entry.instance.clone(),
...
pub(crate) fn ensure_version(version: u32) -> Result<(), HostError> {
    if version == CONFIG_VERSION {
        Ok(())
    } else {
        Err(HostError::UnsupportedVersion { found: version })
    }
}
